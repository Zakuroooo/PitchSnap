...
export default function SignupPage() {
  ...
  const validate = () => {
    ...
    let isValid = true
    if (formData.name.trim().length < 2) { errors.name = "Name must be at least 2 characters"; isValid = false }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) { errors.email = "Please enter a valid email address"; isValid = false }
    if (formData.password.length < 8) { errors.password = "Password must be at least 8 characters"; isValid = false }
    ...
    <div className="min-h-screen flex bg-[#0A0A0A]">
      ...
      <div className="flex-1 lg:w-[40%] flex flex-col px-4 sm:px-8 lg:px-12 relative"
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-[#e5e2e1] tracking-tight mb-1">Create your account</h1>
          <p className="text-sm text-[#4a4455] mb-8">Start winning clients today.</p>

          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            <div className="space-y-1.5">
              <Label htmlFor="name" className="text-xs font-medium text-[#958da0] uppercase tracking-wider">Full Name</Label>
              <Input id="name" name="name" placeholder="John Doe"
                value={formData.name} onChange={handleChange} disabled={loading}
                className={`h-11 bg-white/[0.03] text-[#e5e2e1] placeholder:text-[#2a2a2a] border transition-colors ${fieldErrors.name ? "border-red-500/60" : "border-white/[0.07] focus-visible:border-[#915EFF]/50"}`}
              />
              {fieldErrors.name && <p className="text-red-400/80 text-xs">{fieldErrors.name}</p>}
            </div>
          ...
          <p className="mt-6 text-sm text-[#4a4455]">
            ...
            <Link href="/login" className="text-[#915EFF] hover:text-[#a370ff] transition-colors cursor-pointer font-medium">
              Log in
            </Link>
          </p>
        </motion.div>
